...
class DetectionDistiller_FR(BaseDetector):
    ...
    def __init__(self,
        ...
        super(DetectionDistiller_FR, self).__init__()
    ...
    def forward_train(self, 
        ...
        for i, item_loc in enumerate(self.distill_cfg):
            for item_loss in item_loc.methods:
                ...
                # student_loss[f"{loss_name}_logits"] = res[2]
                # student_loss[f"{loss_name}_instances"] = self.get_instances_loss(
                #     fea_s[i], fea_t[i], img_metas, kwargs["gt_bboxes"]
                # )
