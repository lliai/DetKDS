...
def objective(trial):
    ...
    trans_func2 = trial.suggest_categorical('trans_func2',
                                            ['no','scale_r1','scale_r2','multi_scale_r4','scale'])
    trans_func3 = trial.suggest_categorical('trans_func3',
                                            ['norm_C'])
    # trans_func = trial.suggest_categorical('trans_func',
    #                                         ["drop", "no", \
    #                                          "norm_C", "norm_N", "softmax_N", "softmax_C", "min_max_normalize",  "batchnorm", \
    #                                          "batch", "channel"])
    distance_func = trial.suggest_categorical('distance_func',
                                              ["l1", "l2", "kl", "smooth_l1", "cos", "pear"])
    # gamma_global = trial.suggest_categorical('gamma_global', [0.5, 1, 2, 5, 10])
    # gamma_fbg = trial.suggest_categorical('gamma_fbg', [50, 100, 200])
    # gamma_instances = trial.suggest_categorical('gamma_instances', [0.5, 1, 2, 5, 10])
    # gamma_logits = trial.suggest_categorical('gamma_logits', [0.5, 1, 2, 5, 10])
